...
export default class LessonInfo extends React.Component {
  ...
  static propTypes = {
    ...
    lastViewedModule: ClassroomPropTypes.node.isRequired,
  ...
  render() {
    const {lessons, lastViewedLesson} = this.props;
    const lessonDisplayNumber =
      NodeHelper.getPosition(lessons, lastViewedLesson) + 1;
    const lessonStarted = !!NodeHelper.getCompletionPercentage(
      lastViewedLesson
    ...
    const subtitle = lessonStarted
      ? __('Your Latest Activity')
      : __('Start on');
    const prefix = _.get(lastViewedLesson, 'is_project_lesson')
      ? __('Project')
      : __('Lesson <%= lessonDisplayNumber %>', {lessonDisplayNumber});
    const title = __('<%= prefix %>: <%= lessonTitle %>', {
      prefix,
      lessonTitle: _.get(lastViewedLesson, 'title', '')
    });
      ...
      <div styleName="container">
        <div styleName={this._getStyleWithSuffix('lesson-info')}>
          ...
          <div styleName="title-container">
            <h5 styleName={this._getStyleWithSuffix('lesson-subtitle')}>
              {subtitle}
            </h5>
