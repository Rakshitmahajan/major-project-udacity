...
export class Layout extends React.Component {
  ...
  handleSearch = async (searchTerm) => {
    ...
    if (_.get(searchTerm, 'length', 0) > SEARCH_TERM_LENGTH_THRESHOLD) {
      ...
      } = await onSearch({term: searchTerm, root, partKeys: eligiblePartKeys});
...
export default compose(
  withProps(({root}) => ({
    eligiblePartKeys: _.chain(_.get(root, 'parts'))
      .reject({
        locked_reason: PartLockedReason.SPECIALIZATION_NOT_CHOSEN
      })
      .map('key')
      .value()
  }))
