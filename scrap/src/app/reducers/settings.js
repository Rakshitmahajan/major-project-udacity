...
export default function(state = initialState, action) {
  ...
  switch (action.type) {
    ...
    case Actions.Types.UPDATE_USER_COMPLETED:
      ...
      state = ReducerHelper.merge({}, state, {user});
      break;
    ...
    case Actions.Types.FETCH_ME_COMPLETED:
      ...
      var nanodegreesAndCourses = {
        current: me.current_nanodegrees_and_courses,
        graduated: me.graduated_nanodegrees_and_courses
      };
    ...
    case Actions.Types.FETCH_CURRENT_NANODEGREES_AND_COURSES_COMPLETED:
      const fetchedCurrentNanodegreesAndCourses = action.payload;
      const stateCurrentNanodegreesAndCourses =
        state.user.nanodegreesAndCourses.current;

      var nanodegreesAndCourses = {
        current: _.concat(
          stateCurrentNanodegreesAndCourses,
          fetchedCurrentNanodegreesAndCourses
        )
      };
    ...
    case Actions.Types.FETCH_GRADUATED_NANODEGREES_AND_COURSES_COMPLETED:
      var fetchedGraduatedNanodegreesAndCourses = action.payload;
      var stateGraduatedNanodegreesAndCourses =
        state.user.nanodegreesAndCourses.graduated;
      var nanodegreesAndCourses = {
        graduated: _.concat(
          stateGraduatedNanodegreesAndCourses,
          fetchedGraduatedNanodegreesAndCourses
        )
      };
      state = ReducerHelper.merge({}, state, {
        user: {
    ...
    case Actions.Types.UPDATE_USER_SETTINGS_COMPLETED:
      ...
      state = ReducerHelper.merge({}, state, {user: {settings}});
      break;
    ...
    case Actions.Types.CANCEL_SUBSCRIPTION_COMPLETED: {
      ...
      const idx = _.findIndex(state.subscriptions, {urn: subscription.urn});
      if (idx >= 0) {
        state = update(state, {subscriptions: {[idx]: {$merge: subscription}}});
      }
      break;
    }
    ...
    case Actions.Types.CANCEL_TERM_PURCHASE_COMPLETED: {
      ...
      const idx = _.findIndex(state.termPurchases, {urn: term_purchase.urn});
      if (idx >= 0) {
        state = update(state, {
          termPurchases: {[idx]: {$merge: term_purchase}}
        });
      }
      break;
    }
