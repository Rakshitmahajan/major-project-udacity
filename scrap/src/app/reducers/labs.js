...
export default function(state = {}, action) {
  ...
  switch (action.type) {
    case Actions.Types.FETCH_NANODEGREE_COMPLETED:
      ...
      const labs = ReducerHelper.getLabsForNanodegree(nanodegree);
    ...
    case Actions.Types.UPDATE_LAB_RESULT_COMPLETED:
      ...
      const labKey = _.chain(state)
        .find({id: labResult.lab_id})
        .get('key')
        .value();

      state = ReducerHelper.merge({}, state, {
        [labKey]: {result: _.omit(labResult, 'lab_id')}
      });
      break;
    ...
    case Actions.Types.FETCH_LAB_COMPLETED:
      ...
      state = ReducerHelper.merge({}, state, {[lab.key]: lab});
      break;
  }
